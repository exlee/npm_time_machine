...
pub async fn run(args: CliArgs) {
    ...
    let machine = TimeMachine {
        ...
        reader: Arc::new(PkgReader::from_path(args.input_file.clone().into())),
        args: Arc::new(args)
    };
...
pub fn write_json(&self) {
    ...
    let mut handle = File::create("./output.json").expect("Can't open file for writing");
